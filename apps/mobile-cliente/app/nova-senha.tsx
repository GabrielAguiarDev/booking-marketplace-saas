...
import { MIN_PASSWORD_LENGTH, passwordError } from "../src/auth/validation";
import { color } from "../src/theme/tokens";
import { sans } from "../src/theme/type";
import { CodeField, Field } from "../src/ui/Field";
import { Label, PrimaryButton } from "../src/ui/primitives";

/**
 * Passo 2 da recuperação: código + senha nova, na mesma tela.
 *
 * São duas chamadas em sequência — `verifyOtp` abre uma sessão a partir do
 * código, e `updateUser` troca a senha usando essa sessão. Separar em duas
 * telas deixaria o usuário logado no meio do caminho com a senha antiga ainda
 * valendo, e ele não teria como saber disso.
 */
export default function NovaSenha() {
  ...
  async function submit() {
    ...
    if (updateError) {
      // A sessão já existe neste ponto, mas a senha continua a antiga. Dizer
      // isso é obrigatório: senão o usuário sai achando que trocou.
